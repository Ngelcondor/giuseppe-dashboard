...
- **`cssEntry: .design-sync/build/ds.css`** is Tailwind compiled from `globals.css`. Regenerate with:
  `cat src/app/globals.css .design-sync/_extra.css > .design-sync/build/input.css && npx tailwindcss -c ./tailwind.config.ts -i .design-sync/build/input.css -o .design-sync/build/ds.css && node .design-sync/build/annotate-tw.mjs .design-sync/build/ds.css`
...
- **`--tw-*` are NOT design tokens — marked `/* @kind other */` by `annotate-tw.mjs`.** Tailwind's
  preflight + transform/ring/gradient/shadow utilities emit ~60 internal `--tw-*` custom properties
  (332 declarations) into `ds.css`. The converter appends `cfg.cssEntry` VERBATIM into
  `_ds_bundle.css` (`package-build.mjs` `appendFileSync`, no minify → comments survive), so the
  Claude Design token classifier was flagging every `--tw-*` as an unclassified/unregistered token.
  `.design-sync/build/annotate-tw.mjs` appends `/* @kind other */` after each `--tw-*` declaration
  (idempotent) so the classifier excludes them. It is the LAST step of the regeneration command
  above — **always run it after `tailwindcss`**, or the `--tw-*` reappear as tokens next sync.
  (Can't strip them: the utilities reference them at runtime.)
