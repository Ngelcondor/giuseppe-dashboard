// forked from design-sync lib/dts.mjs — synth-entry (Next.js app, no dist/.d.ts):
// also load the package's own .ts/.tsx sources into the project so <Name>Props
// interfaces resolve via the real checker. ONLY change vs upstream is the
// addSourceFilesAtPaths call in projectFor (marked [synth-entry fork]).
...
function projectFor(pkgDir, typesRoot) {
  ...
  // [synth-entry fork] add the package's .ts/.tsx sources (no shipped .d.ts to read).
  ...
  console.error(`  [DTS] parsed ${project.getSourceFiles().length} source files (.d.ts + src .ts/.tsx) from ${root}`);
...
function isOwnProp(p, pkgDir, styleSystemDirs) {
  ...
  // [synth-entry fork] next bundles a global JSX augmentation (@vercel/og adds
  // `tw` to every HTML element) — never DS API, drop it from every contract.
  if (fp.includes('/next/dist/') || fp.includes('/@vercel/og/')) return false;
